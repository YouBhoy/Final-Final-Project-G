...
type Props = NativeStackScreenProps<FacilitatorMobileStackParamList, 'AssessmentOverrideDetail'>;

export function AssessmentOverrideDetailScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Get assessment definition for title and default maxAttempts
        ...
        if (assessment) {
          if (!cancelled) {
            ...
            // Initialize override to default
        ...
        // Try to get student name from assessment attempts
        const attempts = await assessmentService.getStudentAttempts(assessmentId, studentId);
        if (attempts.length > 0 && !cancelled) {
          setStudentName(attempts[0].studentId === studentId ? `Student ${studentId.slice(-4)}` : 'Student');
        }
      } catch (err) {
        if (!cancelled) {
  ...
  const handleSave = useCallback(async () => {
    ...
    // Validate: override must be at least the number of attempts already used
  ...
  }, [session, overrideValue, attemptCount, assessmentId, studentId, reason, navigation]);
