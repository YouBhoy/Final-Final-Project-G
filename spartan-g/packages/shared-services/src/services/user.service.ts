...
class UserService {
  ...
  async updateCampus(
    ...
    await profileRepository.update(uid, { campus } as Partial<ProfileDocument>);
  }
   ...
   *
   * The campus filter is applied in-memory (over the existing role + isActive
   * query) so no extra Firestore composite index or security-rule change is
   * required. Students may browse facilitators from all campuses or filter by
   * any campus, including their own.
   */
